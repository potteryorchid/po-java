 ...
 * 1.TreeMap（需要排序的时候用）
 *
 * 2.HashMap（无序，根据key的哈希值查找Entry，concurrentHashMap（线程安全）），LinkedHashMap（有序 &&
 * 有序迭代的时候用），HashTable（线程安全）
 *
 *
 ...
 *
 * LinkedList：基于双向列表的机制，插入创建一个Entry对象，并切换前后元素引用，非线程安全。 相对于ArrayList，其优势：add，remove较快，因为只需要操作前后元素，而ArrayList需要操作整个列表，get，set较慢以为ArrayList是有序的，LinkedList需要整个遍历。
 *
 ...
 *
 * LinkedHashSet：根据哈希值来判断元素存贮的位置，同时使用链表来维护元素之前的顺序，所以他是有序的。优势：迭代速度比HashSet好，插入、删除速度稍慢（因为需要维护前后元素的关系）。
 *
 *
 * === Map 集合 ===
 *
